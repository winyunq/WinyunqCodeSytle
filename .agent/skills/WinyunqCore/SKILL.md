...
- `TargetLock.Lock` 锁定的是当前逻辑 Target，不是整个文件；同一文件内未锁定的兄弟函数仍可维护。
- 锁定 Target 仍可执行所有读取和无副作用预览，但 `EditCode.Replace` 必须由服务端拒绝。
- 只有用户明确要求解锁时才可调用 `TargetLock.Unlock`，并必须提交 `UNLOCK <完整 Target 名>` 确认文本。
- 锁定或解锁都会清除旧编辑票据；解锁后必须重新 `EditCode.Prepare`，不能恢复锁定前的写入事务。
- AI 不得为了完成任务自行解锁，不得将“用户要求修改”推断为“用户要求解锁”。
